...
export default function HeatmapPage() {
    ...
    const validateInput = (formData: FormData) => {
        ...
        if (![MIN_REQUIRED_FEATURES, MAX_REQUIRED_FEATURES].includes(selectedFeatures.current.length) || selectedFeatures.current.length > MAX_REQUIRED_FEATURES) {
            setInputError(`Heatmap requires at least ${MIN_REQUIRED_FEATURES} or at most ${MAX_REQUIRED_FEATURES} features to be selected.`)
            return false
        }
    ...
    const generatePlotData = useCallback((outputs: any = []) => {
        ...
        resultKeys.forEach((resultKey) => {
            ...
            outputs.forEach((output: any) => {
                output?.predictions?.[resultKey] && result.range.push(output.predictions[resultKey]) && result.plotData.marker.color.push(output.predictions[resultKey]);

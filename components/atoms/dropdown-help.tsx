      ...
      {({ open }) => (
        <>
          ...
          <Transition
            ...
            leaveTo="opacity-0"
          >
            <div className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase pt-1.5 pb-2 px-3">Get in touch</div>
            <MenuItems as="ul" className="focus:outline-none">
              ...
              <MenuItem as="li">
                {({ active }) => (
                  <Link className={`font-medium text-sm flex items-center py-1 px-3 ${active ? 'text-violet-600 dark:text-violet-400' : 'text-violet-500'}`} 
                    href="mailto:business@leafylab.io?subject=LeafyLab%20Platform"
                  >
                    <svg className="w-3 h-3 fill-current text-violet-500 shrink-0 mr-2" viewBox="0 0 12 12">

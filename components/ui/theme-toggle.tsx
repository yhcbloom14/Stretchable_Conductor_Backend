...
export default function ThemeToggle() {
  ...
  const { isLightModePage, toggleTheme, mounted } = usePageTheme()
  ...
  // Check if theme toggle should be disabled (only on light mode pages)
  const isThemeDisabled = isLightModePage
      ...
      <label 
        className={`flex items-center justify-center w-8 h-8 rounded-full ${
          isThemeDisabled 
            ...
            : 'cursor-pointer hover:bg-gray-100 lg:hover:bg-gray-200 dark:hover:bg-gray-700/50 dark:lg:hover:bg-gray-800'
        }`} 
        htmlFor="light-switch"
        title={isThemeDisabled ? 'Theme locked to light mode on this page' : 'Toggle theme'}
      >
        <svg

...
// Paper interface for compatibility with existing code
...
function transformExtractionToPaper(extraction: Extraction): Paper {
  return {
    response_uid: extraction.sid,
    title: extraction.source_title || '',
    author: '', // Not available in API response
    year: extraction.source_published_year || 0,
    doi: '', // Not available in API response
    publication: extraction.source_journal || '',
    sampleCount: extraction.n_results || 0,
    model: extraction.model || '',
    promptVersion: extraction.prompt_version || '',
    extractionTime: extraction.responsed_at || '',
  };
}
...
export class BioprompApiClient {
  ...
  async getExtractions(params: {
    ...
    page?: number;
    page_size?: number;
  } = {}): Promise<{ items: Paper[], count: number }> {
    ...
    return {
      items: response.items.map(transformExtractionToPaper),

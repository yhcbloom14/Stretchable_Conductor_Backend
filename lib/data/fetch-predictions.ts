...
export const fetchPredictions = async (inputs: any, templateId?: string): Promise<any> => {
    try {
        ...
        throw new Error(`Error fetching predictions: ${response?.data?.message || 'Unknown error'}`)
    } catch (error) {
        console.error(error)
        throw error
    }
